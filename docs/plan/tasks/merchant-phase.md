...
|------|------|--------|---------|------|------|
| M3-001 | 支付相关数据库 Schema 审查与修复 | P0 | 1d | 无 | 修复 M1 `payment_accounts` 表名/字段不匹配；扩展 `payment_account_groups` type ENUM |
| M3-002 | 新增支付相关数据库表迁移 | P0 | 2d | M3-001 | `jh_merchant_payment_group_mappings`、`jh_paypal_safe_descriptions`、`jh_blacklist`、`jh_commission_rules`、`jh_notifications` 表迁移 |

#### Wave 1 — 支付账号池管理

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-003 | 支付账号分组 CRUD + 4 种分组类型 | P0 | 2d | M3-001 | 分组类型：VIP_EXCLUSIVE / STANDARD_SHARED / LITE_SHARED / BLACKLIST_ISOLATED；分组 CRUD API |
| M3-004 | Domain→Merchant→Group 三层映射服务 | P0 | 3d | M3-002, M3-003 | `PaymentGroupMappingService`：域名→查 `jh_stores` 获 merchant_id→查分组映射获 group_id→查分组内可用账号 |
| M3-005 | 账号生命周期管理 | P1 | 2d | M3-003 | `AccountLifecycleService`：4 阶段（new/growing/mature/aging）自动流转 + 阶段限额自动调整 |
| M3-006 | 账号健康度评分 | P0 | 2d | M3-003 | `AccountHealthScoreService`：综合错误率/冻结信号/限制信号实时计算 0-100 分；健康度 <30 自动禁用 |

#### Wave 2 — 选号算法与网关集成

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-007 | 8 层选号算法完整重写 | P0 | 4d | M3-004, M3-006 | `ElectionService`：风控前置→三层映射→可用筛选→优先级→限额→健康度→行为约束→返回/容灾 |
| M3-008 | PayPal 网关集成 | P0 | 4d | M3-007 | 标准支付 + 信用卡直付 + Webhook 处理 + Tracking 上传 |
| M3-009 | Stripe 网关集成 | P0 | 3d | M3-007 | Checkout Session + Webhook 处理 |
| M3-010 | 商品描述脱敏服务 | P0 | 2d | M3-002 | `PayPalDescriptionService`：三层防护（全局映射 + 站点模板 + 动态轮换） |
| M3-011 | 交易行为模拟保护 | P0 | 2d | M3-007 | 金额波动 ±$0.01-$0.99 + 时间限频 + IP 监控 + 退款率预警 |

#### Wave 3 — 佣金与结算

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-012 | 佣金规则引擎 | P0 | 3d | M2-001 | `CommissionService`：基础佣金 - 成交量奖励 - 忠诚度奖励；范围 [8%, 35%] |
| M3-013 | 结算单自动生成 | P0 | 4d | M3-012 | `SettlementService` + `GenerateSettlementJob`：月结，跨 Tenant DB 聚合 |
| M3-014 | 结算审核与付款流程 | P0 | 2d | M3-013 | 状态流转 draft→pending_review→approved→paid；管理员 API |
| M3-015 | 退款/争议对结算影响 | P1 | 2d | M3-013 | 全额扣除、部分按比例、争议冻结不计入 |

#### Wave 4 — 风控与安全

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-016 | 商户风险评分 | P1 | 2d | M2-007 | `MerchantRiskService`：退款率 30% + 争议率 25% + 销量异常 20% + 客诉率 15% + 合规性 10% |
| M3-017 | 动态限额调整 | P1 | 1d | M3-016, M3-006 | 基于风险评分自动调整；高风险下调 50%；极高风险暂停 |
| M3-018 | 平台/商户级黑名单 | P1 | 2d | M3-004, M3-016 | `BlacklistService`：ip/email/device/payment_account；风险评分自动触发 + 手动干预 |

#### Wave 5 — 签名验证与消息推送

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-019 | 支付请求签名验证中间件 | P0 | 2d | M2-011 | `VerifyMerchantSignature`：RSA-SHA256，仅资金操作接口 |
| M3-020 | 签名 SDK/工具类 | P0 | 1.5d | M3-019 | `MerchantSignatureClient` + Guzzle 中间件自动签名 |
| M3-021 | 防重放攻击机制 | P0 | 1d | M3-019 | Timestamp ±5min + Nonce Redis SET NX |
| M3-022 | 消息推送模块 | P1 | 3d | M3-002 | `NotificationService`：站内通知 + 钉钉 Webhook 推送 |

#### Wave 6 — 集成测试

| 编号 | 任务 | 优先级 | 预估工时 | 依赖 | 产出 |
|------|------|--------|---------|------|------|
| M3-023 | Phase M3 集成测试 | P0 | 4d | 所有 M3 任务 | 支付网关 Mock 测试、选号算法、佣金计算精度、结算聚合、RSA 签名、防重放、黑名单规则、消息推送 |
...
| **MM3** 支付结算 | Week 12 | 12 | 三层映射+8层选号正常；PayPal/Stripe 网关集成；商品脱敏三层防护；交易行为模拟保护；佣金实时计算；结算单自动生成并可审核；风控+黑名单；API 签名验证+防重放正常；消息推送 |
...
**Wave 0 — 基础设施修复**
- [ ] `payment_accounts` 表名/字段与代码一致，ENUM 扩展完成
- [ ] 新增 5 张支付相关表迁移成功（group_mappings/safe_descriptions/blacklist/commission_rules/notifications）

**Wave 1 — 支付账号池管理**
- [ ] 支付账号分组 CRUD 正常（4 种类型：VIP_EXCLUSIVE/STANDARD_SHARED/LITE_SHARED/BLACKLIST_ISOLATED）
- [ ] Domain→Merchant→Group 三层映射查询正确
- [ ] 账号生命周期 4 阶段（new/growing/mature/aging）自动流转正常
- [ ] 账号健康度评分 0-100 实时计算，<30 自动禁用

**Wave 2 — 选号算法与网关集成**
- [ ] ElectionService 8 层选号算法正确（风控前置→三层映射→可用筛选→优先级→限额→健康度→行为约束→返回/容灾）
- [ ] PayPal 标准支付 + 信用卡直付正常
- [ ] PayPal Webhook 处理 + Tracking 上传正常
- [ ] Stripe Checkout Session + Webhook 处理正常
- [ ] 商品描述脱敏三层防护生效（全局映射+站点模板+动态轮换）
- [ ] 交易行为模拟保护生效（金额波动+时间限频+IP监控+退款率预警）

**Wave 3 — 佣金与结算**
- [ ] 佣金按订单实时计算，范围 [8%, 35%]
- [ ] 退款时佣金同步扣回
- [ ] 结算单按商户维度聚合所有站点数据（跨 Tenant DB）
- [ ] 结算流程正确（draft→pending_review→approved→paid）
- [ ] 争议订单金额冻结，不计入当期结算

**Wave 4 — 风控与安全**
- [ ] 商户风险评分运行正常（5 维度加权）
- [ ] 高风险商户限额自动下调 50%，极高风险暂停新交易
- [ ] 黑名单（ip/email/device/payment_account）命中后路由到 BLACKLIST_ISOLATED 分组
