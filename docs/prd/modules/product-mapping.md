...
> 优先级：**P0（安全级）** | 版本：v1.0 | 更新日期：2026-04-16
...
|--------|---------|--------|--------|------|
| F-MAP-001 | SKU 前缀自动识别 | P0 | M | 根据 SKU 前 3 位自动判定商品分类 |
| F-MAP-002 | 精确映射管理 | P0 | M | 为单个商品配置专属安全名称 |
| F-MAP-003 | SKU 前缀通用名 | P0 | S | 按前缀分类返回通用安全名称 |
| F-MAP-004 | 兜底默认名配置 | P0 | S | 无匹配时的默认安全名称 |
| F-MAP-005 | 三级优先级引擎 | P0 | M | 精确→前缀→兜底 的查询链 |
| F-MAP-006 | 安全名称库管理 | P1 | S | 管理可用的安全商品名称池 |
| F-MAP-007 | 场景使用控制 | P0 | M | 不同场景调用时返回正确名称 |
...
- [ ] **价格字段在所有场景中未被修改**
- [ ] 买家前台展示使用真实商品名称
- [ ] Facebook Pixel 事件使用真实商品名称和 ID

### 边界场景
- [ ] 当精确映射被删除时，自动回退到 SKU 前缀规则
- [ ] 当安全名称库为空时，兜底名称仍可用
- [ ] 当 SKU 前缀不匹配任何已知分类时，使用兜底默认名
- [ ] 批量商品同时请求安全名称时，性能不退化
