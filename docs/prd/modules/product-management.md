...
**优先级**：P0 | **复杂度**：S | **实现状态**：✅ 已实现（M4）

#### F-PROD-012: 品类特货标识 ✅ 已实现（M4）
...
#### F-PROD-020: 基于品类和品牌的自动特货识别 ✅ 已实现（M4）
...
**优先级**：P0 | **复杂度**：L | **实现状态**：✅ 已实现（M4）

#### F-PROD-031: 多语言安全名称 ✅ 已实现（M4）
...
**优先级**：P1 | **复杂度**：M | **实现状态**：✅ 已实现（M4）

#### F-PROD-032: 安全映射查询优先级（4 级） ✅ 已实现（M4）
...
**优先级**：P0 | **复杂度**：XL | **实现状态**：✅ 已实现（M4）

#### F-PROD-061: 同步触发方式 ✅ 已实现（M4）
...
#### F-PROD-063: 增量同步 vs 全量同步 ✅ 已实现（M4）
...
**优先级**：P2 | **复杂度**：M | **实现状态**：✅ 已实现（M4）

### 5.3 站点级商品差异化

#### F-PROD-070: 站点级价格覆盖 ✅ 已实现（M4）
...
#### F-PROD-080: 按目标市场的展示名称选择 ✅ 已实现（M4）
...
- [x] 可创建一级品类，包含编码、名称（16 种语言）、特货标识
- [x] 可在一级品类下创建二级品类
- [x] 品类编码创建后不可修改
- [x] 删除有关联商品的品类时系统返回错误提示
- [x] 品类列表可按状态筛选，支持树形展示

### 功能验收 — 特货/普货识别 ✅ 已验收（M4）

- [x] 创建 hic 前缀 SKU 的商品时自动标记为特货
- [x] 创建 WPZ 前缀 SKU 的商品时自动标记为普货
- [x] 创建其他前缀 SKU 的商品时根据品牌和品类自动判定
- [x] 管理员可手动覆盖特货标记，操作日志记录完整
- [x] 批量特货标记功能正常
...
- [x] 品类级安全名称库可增删改查
- [x] 安全名称支持 16 种语言
- [x] 映射优先级 4 级查询正确：精确 > SKU 前缀 > 品类 > 兖底
- [x] 不同品类的商品在支付接口使用不同安全名称（非统一 "Sports Jersey"）
- [x] 安全名称使用统计可查

### 功能验收 — 多站点同步 ✅ 已验收（M4）

- [x] 商户可创建主商品并手动同步到指定站点
- [x] 保存时自动同步功能正常（开启 auto_sync 后）
- [x] 选择性同步（目标站点/排除站点）正确执行
- [x] 增量同步只更新指定字段
- [x] 全量同步正确创建/更新站点商品
- [x] 同步日志完整记录，可查询
- [x] 同步失败可重试
...
- [x] 站点级价格覆盖正确生效
- [x] 站点级安全名称覆盖正确生效
- [x] 站点级可用性控制正确（下架的商品在该站点不展示）
- [x] 站点级描述覆盖正确展示

### 安全验收 ✅ 已验收（M4）

- [x] 支付接口（PayPal/Stripe/Antom 等）中的商品名称为映射后的安全名称
- [x] 物流面单中的商品名称为映射后的安全名称
- [x] 价格字段在所有场景下未被映射修改
- [x] Facebook Pixel 追踪使用真实商品名称和价格
- [x] 真实买家（正常模式）前台展示真实品牌名称、真实图片、真实描述
- [x] 检查人员（safe 模式）前台展示安全映射名称、通用化图片、合规描述
- [x] 支付接口中商品描述**始终**使用安全映射名称（不区分访客类型）
- [x] 物流面单/海关申报**始终**使用安全映射名称（不区分访客类型）
- [x] 混合订单中所有商品在支付/物流接口均使用安全名称
- [x] 斗篷安全模式（X-Cloak-Mode: safe）下返回安全内容

### 多语言验收 ✅ 已验收（M4）

- [x] 品类名称切换语言后正确显示对应翻译
- [x] 安全名称在不同语言站点使用对应语言版本
- [x] 缺失翻译时正确回退到英语
- [x] 阿拉伯语(ar)页面 RTL 布局正确

### 边界场景 ✅ 已验收（M4）

- [x] 商品无任何映射记录时正确使用兖底默认名
- [x] 同步目标站点不可用时任务进入失败状态，不影响其他站点
- [x] 并发同步同一商品到同一站点时不产生重复数据
- [x] 大批量同步（1000+ 商品）时队列正常消费，不超时
- [x] 站点数据库连接失败时同步任务记录错误并可重试
