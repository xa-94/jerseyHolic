...
| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-001 | PayPal 标准支付 | P0 | XL | Create Order → Approve → Capture 完整流程 | ✅ 已实现（M3） |
| F-PAY-002 | PayPal 信用卡直付 | P0 | XL | Advanced Credit/Debit Card Payments，Hosted Fields，含 3DS | ✅ 已实现（M3） |
| F-PAY-003 | Stripe 支付 | P0 | L | Checkout Session 模式 | ✅ 已实现（M3） |
| F-PAY-004 | Antom 支付 | **延后** | L | 推迟至后续 Phase | ⏳ 延后至后续 Phase |
| F-PAY-005 | Payssion 本地支付 | **延后** | M | 推迟至后续 Phase | ⏳ 延后至后续 Phase |
| F-PAY-017 | PayPal Tracking API | P0 | M | 发货后上传 tracking number，卖家保护合规 | ✅ 已实现（M3） |
| F-PAY-018 | PayPal 卖家保护合规 | P0 | M | 确保符合 PayPal Seller Protection 全部要求 | ✅ 已实现（M3） |
...
| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-070 | Layer 1 全局品类映射 | P0 | L | 每个商品分类对应一组安全描述，如 basketball_jersey → "Men's Sports Performance Shirt" | ✅ 已实现（M3） |
| F-PAY-071 | Layer 2 站点级模板 | P0 | M | 每个独立站可自定义安全描述风格 | ✅ 已实现（M3） |
| F-PAY-072 | Layer 3 动态轮换 | P0 | M | 同一品类随机从多个安全描述中按权重选取，避免模式识别 | ✅ 已实现（M3） |
| F-PAY-073 | 安全描述管理后台 | P0 | M | 管理 jh_paypal_safe_descriptions 表，CRUD + 权重配置 | ✅ 已实现（M3） |

### 3.3 选号算法（8层筛选 + 三层映射）

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-006 | 支付账号池管理 | P0 | XL | 多账号 CRUD、分组、限额、优先级 | ✅ 已实现（M3） |
| F-PAY-007 | 智能选号引擎 | P0 | XL | 8层筛选 + 三层映射自动选号 | ✅ 已实现（M3） |
| F-PAY-008 | 异常自动禁用 | P0 | L | 3 分钟异常→自动禁用+启用备用 | ✅ 已实现（M3） |
| F-PAY-009 | Webhook 统一处理 | P0 | L | PayPal/Stripe 异步回调验签+订单更新 | ✅ 已实现（M3） |
| F-PAY-010 | 退款 API | P0 | M | 全额/部分退款 | ✅ 已实现（M3） |
| F-PAY-011 | 风控检查 | P0 | M | 黑名单路由（自动触发+手动干预） | ✅ 已实现（M3） |
| F-PAY-012 | 货币转汇 | P0 | M | 非 USD 自动转汇 | ✅ 已实现（M3） |
| F-PAY-014 | 支付通道自动开关 | P1 | M | 账号耗尽自动下架前台支付 | ✅ 已实现（M3） |
| F-PAY-015 | 3D Secure 验证 | P1 | M | PayPal 3DS 回跳处理 | ✅ 已实现（M3） |
| F-PAY-016 | 支付配置查询 | P0 | S | 前台获取支付配置 | ✅ 已实现（M3） |
| F-PAY-019 | 支付记录域名来源 | P0 | S | 每笔支付记录携带 store_domain，追溯订单来源 | ✅ 已实现（M3） |

### 3.4 资金池管理与账号温养

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-030 | 支付账号分组管理 | P0 | L | VIP独占/Standard共享/Lite共享/黑名单隔离四类分组 | ✅ 已实现（M3） |
| F-PAY-031 | 账号温养生命周期 | P0 | L | new(0-30天)→growing(30-90天)→mature(90-180天)→aging(180天+) 4阶段 | ✅ 已实现（M3） |
| F-PAY-032 | 账号健康度评分 | P0 | L | 综合错误率/冻结信号/限制信号的实时评分 | ✅ 已实现（M3） |
| F-PAY-033 | 三层映射引擎 | P0 | L | Domain→Merchant→PaymentAccountGroup，缓存 Redis TTL 5min | ✅ 已实现（M3） |
| F-PAY-034 | 交易行为模拟 | P0 | M | 金额波动、时间分布约束、退款率控制 | ✅ 已实现（M3） |
| F-PAY-035 | 支付成功率监控仪表板 | P2 | M | 按账号/分组/商户/通道多维度实时监控 | ✅ 已实现（M3） |

### 3.5 佣金规则引擎

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-040 | 佣金规则引擎 | P0 | L | 最终佣金 = 基础佣金(等级) - 成交量奖励 - 忠诚度奖励 | ✅ 已实现（M3） |
| F-PAY-041 | 结算单自动生成 | P0 | XL | 按商户维度聚合所有站点交易，跨 Tenant DB | ✅ 已实现（M3） |
| F-PAY-042 | 结算周期管理 | P0 | M | 初期仅月结，结算周期作为配置项预留扩展 | ✅ 已实现（M3） |
| F-PAY-043 | 退款/争议结算影响 | P0 | L | 全额/部分退款扣减 + 争议冻结 | ✅ 已实现（M3） |
| F-PAY-044 | 结算对账 | P1 | L | 系统流水 vs 支付网关对账 | ✅ 已实现（M3） |
| F-PAY-045 | 结算报表 | P2 | M | 按商户/站点/品类/时间维度的结算汇总 | ✅ 已实现（M3） |

### 3.6 风控与黑名单

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-050 | 商户风险评分 | P0 | L | 聚合所有站点的争议率/退款率/拒付金额 | ✅ 已实现（M3） |
| F-PAY-051 | 动态限额调整 | P0 | M | 基于风险评分自动调整日/月限额 | ✅ 已实现（M3） |
| F-PAY-052 | 买家行为风控 | P2 | L | 重复下单检测、大额预警、设备指纹 | ✅ 已实现（M3） |
| F-PAY-053 | 平台级黑名单管理 | P0 | M | 风险评分自动触发 + 管理员手动干预，维度：ip/email/device/payment_account | ✅ 已实现（M3） |
| F-PAY-054 | 商户级黑名单管理 | P1 | S | 商户自行维护站点范围黑名单 | ✅ 已实现（M3） |
| F-PAY-055 | 黑名单双级范围 | P0 | M | platform(全局) + merchant(商户级) 两级范围 | ✅ 已实现（M3） |

### 3.7 RSA 签名验证（仅资金操作接口）

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-060 | VerifyMerchantSignature 中间件 | P0 | L | 验证资金操作接口的 RSA-SHA256 数字签名 | ✅ 已实现（M3） |
| F-PAY-061 | MerchantSignatureClient SDK | P0 | M | PHP 签名 SDK + Guzzle 中间件自动签名 | ✅ 已实现（M3） |

### 3.8 消息推送模块

| 功能ID | 功能名称 | 优先级 | 复杂度 | 描述 | 实现状态 |
|--------|---------|--------|--------|------|----------|
| F-PAY-080 | 站内通知系统 | P0 | M | 数据库记录 + 管理后台轮询/WebSocket | ✅ 已实现（M3） |
| F-PAY-081 | 钉钉 Webhook 推送 | P0 | M | 钉钉机器人，覆盖风险告警、结算提醒、账号异常 | ✅ 已实现（M3） |
| F-PAY-082 | NotificationService 统一服务 | P0 | M | 统一消息发送入口，支持多通道分发 | ✅ 已实现（M3） |
