...
> 版本：v3.0 (Phase M3) | 更新日期：2026-04-17
> 本文档定义跨模块通用的核心业务规则，各模块 PRD 应引用此处规则 ID。
> 规则系列：BR-MAP、BR-ORD、BR-PAY、BR-SHIP、BR-RISK、BR-I18N、BR-CLK、BR-CURR、BR-MCH、BR-SEC、BR-PAY-POOL、BR-PAY-SETTLE、BR-PAY-RISK、BR-PROD、BR-MIX、BR-MULTI-STORE、**BR-PAY-DESC**、**BR-PAY-NURTURE**、**BR-PAY-COMM**、**BR-PAY-NOTIFY**
...
### BR-MCH-005: 商户等级与佣金（Phase M3 更新）
...
**Phase M3 佣金计算公式**（参见 BR-PAY-COMM-001）：
```
最终佣金 = 基础佣金(等级) - 成交量奖励(0~5%) - 忠诚度奖励(0~3%)
范围约束：min 8%, max 35%
```
...
### BR-PAY-POOL-002: 账号生命周期阶段（Phase M3 温养策略）

- **触发条件**：账号创建、系统定时评估时
- **规则内容**：

```
new(0-30天) → growing(30-90天) → mature(90-180天) → aging(180天+)
                                              → suspended(暂停) → growing/aging
```

| 阶段 | 时间范围 | 日限额 | 月限额 | 单量约束 |
|------|----------|--------|--------|----------|
| 新建 (new) | 0-30天 | $200 | $2,000 | 每天最多3-5单，单笔≤$50 |
| 成长 (growing) | 30-90天 | $500 | $8,000 | 每天10-20单 |
| 成熟 (mature) | 90-180天 | $2,000 | $30,000 | 正常分配 |
| 老化 (aging) | 180天+ | $5,000 | $50,000 | 正常分配 |

**交易行为模拟约束**（参见 BR-PAY-NURTURE-002）：
- 金额波动：±$0.01-$0.99 随机浮动
- 时间分布：同账号60秒内最多1笔，5分钟内最多3笔
- 退款控制：退款率>1%预警，>1.5%账号降级

- **安全等级**：高
- **示例**：新账号第15天，日限额$200，每天最多5单，单笔不超过$50 → 第31天自动升级为 growing，日限额提升到$500。
...
### BR-PAY-SETTLE-001: 结算周期规则（Phase M3：初期仅月结）

- **触发条件**：结算周期到期自动触发
- **规则内容**：

> **Phase M3 范围**：初期仅支持月结，结算周期作为配置项预留扩展。以下多周期方案作为未来扩展参考：

| 商户等级 | 结算周期 | 说明 |
|---------|---------|------|
| 入门级 | 月结（T+30） | 新商户默认 |
| 中级 | 月结（T+30） | M3 初期统一月结 |
| 高级 | 月结（T+30） | M3 初期统一月结 |
| VIP | 月结（T+30） | M3 初期统一月结 |

**结算流程**：定时生成 draft → 管理员审核 → approved → 打款 → paid

**跨 Tenant DB 聚合**：遍历商户名下所有站点数据库，聚合订单数据

- **安全等级**：高
- **示例**：标准商户月均 $8,000 → M3 初期统一月结，未来可扩展为双周结。
...
### BR-PAY-SETTLE-003: 退款/争议对结算的影响（Phase M3 更新）

- **触发条件**：退款/争议事件发生时
- **规则内容**：
  - 全额退款：扣除订单金额 **和佣金** 均从结算中扣除
  - 部分退款：按退款比例扣减
...
## BR-PAY-DESC: 商品描述脱敏规则 *(M3 新增)*

### BR-PAY-DESC-001: 三层安全描述匹配

- **触发条件**：支付下单时，系统为 PayPal/Stripe 请求生成商品描述
- **规则内容**：
  三层匹配优先级（高→低）：
  1. **Layer 1 — 站点级模板**：查询 `jh_paypal_safe_descriptions` 中 `store_id = 当前站点` 的记录，若命中则使用
  2. **Layer 2 — 全局品类映射**：查询 `store_id IS NULL` 的全局记录，按 `product_category` 匹配
  3. **Layer 3 — 兜底默认**：若上述均无匹配，使用系统硬编码兜底描述 `"General Merchandise"`
- **安全等级**：最高
- **示例**：站点 A 自定义了 basketball_jersey 的描述 "Men's Athletic Training Top" → 优先使用。站点 B 未自定义 → 回退全局映射 "Men's Sports Performance Shirt"。

### BR-PAY-DESC-002: 动态轮换（权重随机）

- **触发条件**：同一 store_id + product_category 存在多条 status=active 的安全描述
- **规则内容**：
  - 按 `weight` 字段加权随机选取一条描述
  - 权重公式：`P(i) = weight(i) / Σweight`
  - 目的：避免同一描述被频繁使用，降低支付平台模式识别风险
- **安全等级**：最高
- **示例**：全局 basketball_jersey 有 3 条描述，权重分别为 50/30/20 → 选中概率 50%/30%/20%。

### BR-PAY-DESC-003: 安全描述管理约束

- **触发条件**：管理员通过后台 CRUD 安全描述
- **规则内容**：
  - 每个 `store_id + product_category` 组合下至少保留 1 条 active 记录（不可全部禁用）
  - 全局（store_id=NULL）每个 product_category 建议 ≥ 3 条，确保轮换多样性
  - safe_description 长度限制 ≤ 127 字符（PayPal 描述字段限制）
  - safe_name 长度限制 ≤ 200 字符
- **安全等级**：高
- **示例**：管理员尝试禁用 basketball_jersey 全局最后一条 active 描述 → 系统拒绝并提示 "至少保留一条有效描述"。

---

## BR-PAY-NURTURE: 账号温养规则 *(M3 新增)*

### BR-PAY-NURTURE-001: 生命周期阶段与限额

- **触发条件**：支付账号参与选号算法时
- **规则内容**：
  根据账号 `lifecycle_days`（自首次激活起累计天数）自动划分阶段并应用对应限额：

  | 阶段 | 天数范围 | 日限额 | 月限额 | 日单量 | 单笔上限 |
  |------|---------|--------|--------|--------|----------|
  | new | 0-30 | $200 | $2,000 | 3-5 | $50 |
  | growing | 30-90 | $500 | $8,000 | 10-20 | — |
  | mature | 90-180 | $2,000 | $30,000 | — | — |
  | aging | 180+ | $5,000 | $50,000 | — | — |

  阶段升级由每日定时任务（Cron）自动更新 `lifecycle_stage` 字段。
- **安全等级**：最高
- **示例**：账号激活 45 天 → growing 阶段，日限额 $500。当天已收款 $480 → 选号算法跳过该账号。

### BR-PAY-NURTURE-002: 交易行为模拟约束

- **触发条件**：选号算法最终返回账号前
- **规则内容**：
  为模拟真实交易行为、降低支付平台风控触发概率，需满足以下约束：
  1. **金额波动**：实际扣款金额 = 订单金额 ± 随机浮动（$0.01 ~ $0.99）
  2. **频率控制**：同一账号 60 秒内最多 1 笔交易，5 分钟内最多 3 笔
  3. **退款率监控**：
     - 退款率 > 1%：触发预警通知（站内 + 钉钉）
     - 退款率 > 1.5%：自动降级账号（health_score 减 20，lifecycle_stage 回退一级）
     - 退款率 > 3%：自动冻结账号（status → frozen）
  4. **健康度检查**：health_score < 30 的账号不参与选号
- **安全等级**：最高
- **示例**：订单金额 $29.99 → 实际扣款 $30.37（+$0.38）。账号最近一笔交易在 40 秒前 → 等待至 60 秒后再执行。

---

## BR-PAY-COMM: 佣金规则 *(M3 新增)*

### BR-PAY-COMM-001: 三维佣金计算公式

- **触发条件**：结算单生成时计算商户佣金
- **规则内容**：
  ```
  最终佣金率 = 基础佣金率(等级) - 成交量奖励 - 忠诚度奖励
  约束：max(8%, min(35%, 最终佣金率))
  ```
...
- **安全等级**：高
- **示例**：Standard 商户（基础 18%），月成交 $35k（-3%），合作 8 个月（-2%）→ 最终佣金率 = 18% - 3% - 2% = 13%。

### BR-PAY-COMM-002: 佣金结算时机

- **触发条件**：结算周期到达时
- **规则内容**：
  - 佣金在结算单生成时统一计算，不在每笔交易时实时扣除
  - 结算单中 `commission_amount = Σ(订单金额 × 最终佣金率)`
  - 退款订单的佣金需同步扣回：退款后佣金 = 原佣金 × (1 - 退款比例)
  - 争议中订单的佣金暂时冻结，争议解决后按结果处理
- **安全等级**：高
- **示例**：结算周期内 100 笔订单，其中 2 笔全额退款、1 笔部分退款 50% → 佣金计算排除 2 笔全额退款订单，部分退款订单佣金减半。

---

## BR-PAY-NOTIFY: 消息推送规则 *(M3 新增)*

### BR-PAY-NOTIFY-001: 通知触发与通道策略
...
- **规则内容**：
  ...
  **推送规则**：
  - 站内通知：写入 `jh_notifications` 表，前端轮询或 WebSocket 推送
  - 钉钉推送：通过 Webhook 机器人发送，仅 warning/critical 级别触发
  - 失败重试：最多 3 次，间隔 30 秒
  - 通知去重：同一事件 10 分钟内不重复推送（Redis SET NX TTL 10min）
- **安全等级**：中
- **示例**：账号 PA-001 退款率达到 1.2% → 触发 warning 级别通知 → 站内通知写入 DB + 钉钉 Webhook 推送给运营群。
