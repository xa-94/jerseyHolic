...
return new class extends Migration
{
    ...
    {
        // 支付记录
        Schema::create('jh_payments', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('order_id')->comment('订单ID');
            $table->string('payment_no', 64)->comment('支付流水号');
            $table->unsignedBigInteger('account_id')->default(0)->comment('支付账号ID');
        ...
        // 支付交易流水
        Schema::create('jh_payment_transactions', function (Blueprint $table) {
            $table->id();
        ...
        // 退款记录
        Schema::create('jh_refunds', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('order_id')->comment('订单ID');
        ...
        // 争议记录
        Schema::create('jh_disputes', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('order_id')->comment('订单ID');
        ...
        // 信用卡临时信息
        Schema::create('jh_payment_cards', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('order_id')->comment('订单ID');
    ...
    {
        Schema::dropIfExists('jh_payment_cards');
        Schema::dropIfExists('jh_disputes');
        Schema::dropIfExists('jh_refunds');
        Schema::dropIfExists('jh_payment_transactions');
        Schema::dropIfExists('jh_payments');
    }
};
